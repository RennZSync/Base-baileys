export default function (m, conn) {
	try {
		...
		if (m.sender.endsWith('@s.whatsapp.net')) {
			if (!global.db.data.users[m.sender]) global.db.data.users[m.sender] = { ...defaultUser };
			else (Object.assign(defaultUser, global.db.data.users[m.sender]), (global.db.data.users[m.sender] = defaultUser));
		}

		// === GROUP ===
		if (m.isGroup) {
			if (!global.db.data.chats[m.chat]) global.db.data.chats[m.chat] = { ...defaultChat };
			else (Object.assign(defaultChat, global.db.data.chats[m.chat]), (global.db.data.chats[m.chat] = defaultChat));
		}

		// === SETTINGS ===
		if (!global.db.data.settings[conn.user.jid]) global.db.data.settings[conn.user.jid] = { ...defaultSettings };
