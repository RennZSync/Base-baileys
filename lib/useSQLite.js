...
import { BufferJSON, initAuthCreds, proto } from '@rennzsync/baileys';
...
export default async (folder = './sessions') => {
	...
	const getCache   = (cat) => { if (!cache.has(cat)) cache.set(cat, new Map()); return cache.get(cat); };
	const getDirty   = (cat) => { if (!dirty.has(cat)) dirty.set(cat, new Set()); return dirty.get(cat); };
	const getMutex   = (cat) => { if (!mutexes.has(cat)) mutexes.set(cat, new Mutex()); return mutexes.get(cat); };
	...
	const flushCategory = (category) => {
		...
		dbs.get(category).transaction(() => {
			for (const id of ids) {
				...
				value !== undefined
					? set.run(id, JSON.stringify(value, BufferJSON.replacer), now)
					: del.run(id);
			}
		})();
		ids.clear();
	...
	const flushAll = () => {
		for (const category of dirty.keys()) {
			try { flushCategory(category); }
			catch (err) { console.error(`[auth] flush error [${category}]:`, err); }
		}
	};
